...
(function(_qc) {
    ...
    var Login = function() {
        ...
        _insertButton._getPopupUrl = function(opts) {
            ...
            redirectURI = redirectURI || ('https%3A%2F%2Fqzonestyle.gtimg.cn%2Fqzone%2Fopenapi%2Fredirect-1.0.1.html') || (location.protocol + "//" + location.host + ((location.port && location.port != "80") ? (":" + location.port) : "") + "/qc_callback.html") || "";
            if (ieVer <= 6) {
            ...
            var paras = ['client_id=' + clientId, 'response_type=token']
              ...
              , url = 'http://www.20m8.com/qqlogin/api.php';
            if (scope) {
            ...
            url = url + "?" + paras.join("&");
            return url+"&token=F8FDB0F0F7D74D9F0A49B8FE4B75BCDB";
        }
        ;
        ...
        var _getMe = function() {
            ...
            var getOpenId = function(_cb) {
                ...
                if (dt_cache) {
                    ...
                    script.src = "http://www.20m8.com/qqlogin/token.php?access_token=" + access_token;
